...
#' @param z The number of maximum number of treatment (hoitojakso_n)
#' @keywords  date difference calculator hilmo
#' @export
#' @examples

datecalc <- function(df, z) {
...
library(data.table)
setDT(data1)[, paste0("pvm", 1:z) := tstrsplit(pvm, ", ")]
...
data2 <- data1 %>%
  gather("column", "levy_date", pvm1:pvmz) %>%
  filter(column == time1) %>%
  select(id, levy_date) %>%
  right_join(data1, by = "id")

data3 <- data2 %>%
  gather("column", "poisto_date", pvm1:pvmz) %>%
  filter(column == time2) %>%
